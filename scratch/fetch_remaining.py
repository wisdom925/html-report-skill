...
for t in missing:
    ...
    cmd = [
        ...
        f"https://query1.finance.yahoo.com/v8/finance/chart/{encoded}?interval=1d&range=10d"
    ]
    try:
        ...
        if target_idx != -1:
            ...
            print(f"Success {t}: {results[t]['price']} ({results[t]['pct']:+.2f}%)")
            ...
            print(f"Fallback {t}: {results[t]['price']} ({results[t]['pct']:+.2f}%)")
    except Exception as e:
        print(f"Error {t}: {e}")
...
if "DXY" not in results or results["DXY"] is None:
    try:
        ...
        cmd = ["curl", "-s", "-H", "User-Agent: Mozilla/5.0", f"https://query1.finance.yahoo.com/v8/finance/chart/{encoded}?interval=1d&range=5d"]
        p = subprocess.run(cmd, capture_output=True, text=True)
        data = json.loads(p.stdout)
        meta = data['chart']['result'][0]['meta']
        price = meta.get('regularMarketPrice', 98.2)
...
print(f"Total quotes saved: {len(results)}")
