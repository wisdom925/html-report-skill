...
def fetch_ticker_curl(ticker):
    encoded = urllib.parse.quote(ticker)
    url = f"https://query1.finance.yahoo.com/v8/finance/chart/{encoded}?interval=1d&range=10d"
    cmd = [
        "curl", "-s", "--max-time", "6",
        "--resolve", "query1.finance.yahoo.com:443:180.222.119.247",
...
with ThreadPoolExecutor(max_workers=8) as executor:
    futures = {executor.submit(fetch_ticker_curl, t): t for t in missing}
    for fut in as_completed(futures):
        ...
        try:
            ...
            if res and res[1] is not None:
                results[res[0]] = res[1]
                print(f"Got {res[0]}: {res[1]['price']} ({res[1]['pct']:+.2f}%)")
            else:
...
if "DXY" not in results or results["DXY"] is None:
    _, dxy_res = fetch_ticker_curl("DX-Y.NYB")
    if dxy_res:
        dxy_res["symbol"] = "DXY"
        results["DXY"] = dxy_res
...
print(f"Saved {len(results)} total quotes to {quotes_file}")
