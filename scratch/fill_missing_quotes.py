...
for ticker in missing:
    ...
    url = f"https://query2.finance.yahoo.com/v8/finance/chart/{encoded}?interval=1d&range=10d"
    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            if target_idx != -1:
                ...
                print(f"Fetched {ticker}: {results[ticker]['price']} ({results[ticker]['pct']:+.2f}%)")
                ...
                print(f"Fetched (fallback) {ticker}: {results[ticker]['price']} ({results[ticker]['pct']:+.2f}%)")
    except Exception as e:
...
# DXY fallback using DX-Y.NYB
if "DXY" not in results or results["DXY"] is None:
    try:
        time.sleep(0.3)
        encoded = urllib.parse.quote("DX-Y.NYB")
        url = f"https://query2.finance.yahoo.com/v8/finance/chart/{encoded}?interval=1d&range=5d"
        req = urllib.request.Request(url, headers=headers)
        with urllib.request.urlopen(req, timeout=10) as resp:
            data = json.loads(resp.read().decode())
            meta = data['chart']['result'][0]['meta']
            p = meta.get('regularMarketPrice', 98.2)
            ...
            print("Fetched DXY via DX-Y.NYB")
    except Exception as e:
        print("DXY fetch error:", e)

with open(quotes_file, "w", encoding="utf-8") as f:
    json.dump(results, f, ensure_ascii=False, indent=2)

print(f"Successfully saved {len(results)} total quotes to {quotes_file}")
