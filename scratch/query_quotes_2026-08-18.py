    ...
    try:
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            for i, (t, c, date_str) in enumerate(valid):
                if date_str == '2026-08-18':
                    target_idx = i
                    break
            
            if target_idx == -1 and len(valid) >= 1:
                last_date = valid[-1][2]
                if last_date in ['2026-08-18', '2026-08-19']:
...
with open("/Users/wisdom/html-report-skill/scratch/quotes_2026-08-18.json", "w", encoding="utf-8") as f:
    json.dump(results, f, ensure_ascii=False, indent=2)

print(f"Quotes for 2026-08-18 written to scratch/quotes_2026-08-18.json. Total count: {len(results)}")
